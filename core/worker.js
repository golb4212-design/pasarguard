...
const RELEASE_NOTES = Object.freeze({
  "3.3.47": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "✨", text: "فقط یک نمونه با آیکن پرمیوم جایگزین می‌شود و نمونه‌های دیگر حذف نمی‌شوند" },
      { emoji: "🛡", text: "حفظ تبدیل ایموجی‌های داخل متن پیام‌ها و کپشن‌ها به حالت پرمیوم" }
    ]),
    reseller: Object.freeze([
      { emoji: "🙂", text: "ایموجی دوم کلید دیگر حذف نمی‌شود" },
      { emoji: "ℹ️", text: "هر کلید یک آیکن پرمیوم دارد و ایموجی‌های تکراری دیگر در متن کلید باقی می‌مانند" }
    ])
  }),
  ...
  "3.3.45": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🧩", text: "ساخت خودکار MessageEntity یا تگ HTML معتبر با حفظ قالب‌بندی پیام" },
      { emoji: "🛡", text: "Fallback امن به ایموجی معمولی در صورت ردشدن Custom Emoji توسط تلگرام" }
    ]),
    reseller: Object.freeze([
      { emoji: "💬", text: "نمایش ایموجی پرمیوم ثبت‌شده در عنوان‌ها، توضیحات و پیام‌های مراحل خرید" },
      { emoji: "✅", text: "ذخیره Alt Emoji واقعی ارسالی نماینده برای رندر صحیح Custom Emoji" }
    ])
  }),
  ...
  "3.3.28": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🧭", text: "مسیریابی امن دکمه‌های متنی به عملیات قبلی بدون نیاز به Callback شیشه‌ای" },
      { emoji: "🧹", text: "حذف منوی WebApp شیشه‌ای و استفاده از منوی فرمان‌ها و کیبورد داخلی" }
    ]),
    reseller: Object.freeze([
      { emoji: "📱", text: "تبدیل منوهای مشتری، مالک و مستر به Reply Keyboard پایدار" },
      { emoji: "🔗", text: "نمایش لینک‌ها و عملیات کپی از طریق پیام معمولی به‌جای دکمه شیشه‌ای" },
      { emoji: "🛡", text: "ثبت مسیر دکمه‌های پویا در D1 برای پلن، سرویس، سفارش و مدیریت" }
    ])
  }),
  ...
  "3.3.23": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🖥", text: "پذیرش آدرس مرزبان با یا بدون /dashboard و ساخت لینک ورود مدیریتی صحیح" },
      { emoji: "🔒", text: "عدم کپی UUID، رمز و شناسه‌های امنیتی اشتراک نمونه هنگام ساخت سرویس جدید" }
    ]),
    reseller: Object.freeze([
      { emoji: "⚙️", text: "سرویس‌های جدید نماینده به‌صورت خودکار از Inboundهای اشتراک نمونه مرکزی استفاده می‌کنند" },
      { emoji: "🛡", text: "اطلاعات ورود و لینک داشبورد مرزبان فقط برای مدیر مرکزی قابل مشاهده است" }
    ])
  }),
  ...
  "3.3.20": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🔁", text: "بازسازی خودکار Topic بکاپ در صورت بسته یا حذف شدن و تلاش مجدد برای ارسال فایل" },
      { emoji: "🧩", text: "پشتیبانی یکسان از بکاپ دستی در Core و Edge و بکاپ خودکار در Processor" }
    ]),
    reseller: Object.freeze([
      { emoji: "📎", text: "فایل بکاپ بعد از ساخت مستقیماً در موضوع بکاپ گروه گزارش ارسال می‌شود" },
      { emoji: "✅", text: "نتیجه ارسال فایل به گروه مرکزی و گروه نماینده در گزارش بکاپ نمایش داده می‌شود" },
      { emoji: "🛠", text: "در صورت خراب بودن Topic بکاپ، موضوع به‌صورت خودکار بازسازی می‌شود" }
    ])
  }),
  ...
  "3.3.18": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🛡", text: "اعتبارسنجی BotFather، جلوگیری از توکن تکراری و جابه‌جایی اتمیک Webhook با Secret تازه" }
    ]),
    reseller: Object.freeze([
      { emoji: "🤖", text: "امکان تعویض توکن بدون حذف کاربران، سفارش‌ها، پلن‌ها، موجودی‌ها و تنظیمات ربات" },
      { emoji: "🧹", text: "خارج‌کردن Webhook ربات قبلی و پاک‌سازی کش توکن پس از تعویض موفق" }
    ])
  }),
  ...
  "3.3.15": Object.freeze({
    central: Object.freeze([
      ...
      { emoji: "🔄", text: "بازطراحی بروزرسانی خودکار به‌صورت مرحله‌ای و قابل‌ادامه تا هیچ Worker پشت مرحله قبلی گیر نکند" },
      { emoji: "🧬", text: "تشخیص Build هم‌نسخه با SHA فایل‌ها و همگام‌سازی واقعی Core، Edge و Processor" },
      { emoji: "⚡", text: "فعال‌سازی بررسی بروزرسانی از درخواست‌های پنل در کنار Cron و آپلود ZIP" }
    ]),
    reseller: Object.freeze([
      { emoji: "🖥", text: "آپدیت پایدار پنل‌ها و ربات‌های نماینده حتی وقتی نسخه عددی تغییر نکرده اما کد جدید منتشر شده است" }
    ])
  }),
...
CREATE INDEX IF NOT EXISTS idx_telegram_reply_routes_expiry ON telegram_reply_routes(expires_at);
...
function publicSettings(settings) {
  return {
    ...
    plisio_rate_mode: plisioRateMode(settings),
    plisio_fx_last_fetched_at: settings.plisio_fx_last_fetched_at || "",
...
function adminSettings(settings) {
  const result = {
    ...
    plisio_rate_mode: plisioRateMode(settings),
    plisio_fx_provider: settings.plisio_fx_provider || "navasan",
    plisio_fx_key_configured: Boolean(settings.plisio_fx_api_key),
    plisio_fx_item: settings.plisio_fx_item || "usd_sell",
    plisio_fx_unit: settings.plisio_fx_unit || "toman",
    plisio_fx_refresh_minutes: Number(settings.plisio_fx_refresh_minutes || 15),
    plisio_fx_max_stale_minutes: Number(settings.plisio_fx_max_stale_minutes || 1440),
    plisio_fx_markup_percent: Number(settings.plisio_fx_markup_percent || 0),
    plisio_fx_last_rate_toman: Number(settings.plisio_fx_last_rate_toman || 0),
    plisio_fx_last_raw_rate: Number(settings.plisio_fx_last_raw_rate || 0),
    plisio_fx_last_fetched_at: settings.plisio_fx_last_fetched_at || "",
    plisio_fx_last_error: settings.plisio_fx_last_error || "",
    plisio_allowed_currencies: settings.plisio_allowed_currencies || "",
    plisio_expire_minutes: Number(settings.plisio_expire_minutes || 60),
    ...
    marzban_template_username: settings.marzban_template_username || "",
    marzban_dashboard_url: sharedMarzbanDashboardUrl(settings.marzban_panel_url || ""),
...
function telegramUiFallbackMessagePayload(payload, replyMarkup = undefined) {
  ...
  if (hasText && Array.isArray(payload.entities)) fallback.entities = payload.entities;
  else if (!hasText && Array.isArray(payload.caption_entities)) fallback.entities = payload.caption_entities;
...
let plisioFxRefreshPromise = null;

function normalizeDigitsForNumber(value) {
...
function parseFxNumber(value) {
  const parsed = Number(normalizeDigitsForNumber(value));
  return Number.isFinite(parsed) ? parsed : 0;
}

function plisioRateMode(settings) {
  return String(settings?.plisio_rate_mode || "auto").toLowerCase() === "manual" ? "manual" : "auto";
}

function plisioAutoRateConfigured(settings) {
  return !!String(settings?.plisio_fx_api_key || "").trim() || Number(settings?.plisio_fx_last_rate_toman || 0) > 0;
}

function plisioConfigured(settings) {
  const rateReady = plisioRateMode(settings) === "manual"
    ? Number(settings?.plisio_toman_per_source_unit || 0) > 0
    : (plisioAutoRateConfigured(settings) || Number(settings?.plisio_toman_per_source_unit || 0) > 0);
  return settings?.plisio_enabled === "true" && !!String(settings?.plisio_api_key || "").trim() && rateReady;
}

async function centralPlisioFxApiKey(settings, env) {
  const raw = String(settings?.plisio_fx_api_key || "").trim();
  if (!raw) throw new Error("API Key سرویس نرخ ارز ثبت نشده است");
  ...
  try {
    ...
    throw new Error("API Key سرویس نرخ ارز قابل بازیابی نیست؛ آن را دوباره ثبت کنید");
  }
}

function fxCacheAgeMinutes(settings) {
...
async function storeCentralFxState(env, values) {
  ...
  if (values.marzban_panel_url !== undefined) {
    values.marzban_panel_url = normalizeSharedMarzbanPanelUrl(values.marzban_panel_url);
...
async function fetchNavasanUsdTomanRate(settings, env) {
  const apiKey = await centralPlisioFxApiKey(settings, env);
  ...
  const raw = parseFxNumber(entry?.value ?? entry?.price ?? entry?.rate ?? entry);
  ...
  return { rate_toman: rateToman, raw_rate: raw, item, unit, markup_percent: markup, provider: "navasan" };
}

async function refreshCentralPlisioFxRate(env, { force = false } = {}) {
  const initial = await getSettings(env);
  if (plisioRateMode(initial) !== "auto") {
    const manual = Number(initial.plisio_toman_per_source_unit || 0);
    if (manual <= 0) throw new Error("نرخ دستی Plisio ثبت نشده است");
    return { rate_toman: manual, source: "manual", stale: false, fetched_at: initial.plisio_fx_last_fetched_at || "" };
  }
  ...
  if (!force && cached > 0 && fxCacheAgeMinutes(initial) < refreshMinutes) {
    return { rate_toman: cached, source: "cache", stale: false, fetched_at: initial.plisio_fx_last_fetched_at || "" };
  }
  if (plisioFxRefreshPromise) return plisioFxRefreshPromise;
  plisioFxRefreshPromise = (async () => {
    try {
      const result = await fetchNavasanUsdTomanRate(initial, env);
      const fetchedAt = nowIso();
      await storeCentralFxState(env, {
      ...
      return { ...result, source: "navasan", stale: false, fetched_at: fetchedAt };
    } catch (error) {
      await storeCentralFxState(env, { plisio_fx_last_error: cleanText(error.message, 800) }).catch(() => null);
      ...
      if (last > 0 && fxCacheAgeMinutes(current) <= maxStale) {
        return { rate_toman: last, source: "stale_cache", stale: true, warning: error.message, fetched_at: current.plisio_fx_last_fetched_at || "" };
      }
      const fallback = Number(current.plisio_toman_per_source_unit || 0);
      if (fallback > 0) return { rate_toman: fallback, source: "manual_fallback", stale: true, warning: error.message, fetched_at: "" };
      throw error;
    } finally {
      plisioFxRefreshPromise = null;
    }
  })();
...
function plisioSourceAmount(amountToman, rateToman) {
...
async function testCentralPlisioConnection(env) {
  ...
  const fx = await refreshCentralPlisioFxRate(env, { force: plisioRateMode(settings) === "auto" });
...
async function createPlisioPaymentInvoice(request, env, ctx = null) {
  ...
  const amountRial = amountToman * 10;
  const ts = nowIso();
  const storedPayload = { request: { ...params, api_key: undefined }, response: provider, fx: { rate_toman: fx.rate_toman, source: fx.source, stale: !!fx.stale, fetched_at: fx.fetched_at || "" } };
  try {
    await env.PASARGUARD_DB.prepare(`
...
async function saveAdminSettings(request, env) {
  ...
  const allowed = [
    ...
    "telegram_webhook_secret", "service_backend", "marzban_shared_enabled", "marzban_panel_url", "marzban_admin_username", "marzban_admin_password", "marzban_proxies_json", "marzban_inbounds_json", "marzban_template_username", "marzban_price_per_gb", "marzban_suspend_on_debt", "marzban_sync_batch", "pasarguard_panel_url", "pasarguard_access_token",
  ...
  if (values.plisio_fx_refresh_minutes !== undefined) values.plisio_fx_refresh_minutes = String(Math.max(5, Math.min(1440, Number(values.plisio_fx_refresh_minutes || 15))));
  if (values.plisio_fx_max_stale_minutes !== undefined) values.plisio_fx_max_stale_minutes = String(Math.max(15, Math.min(10080, Number(values.plisio_fx_max_stale_minutes || 1440))));
  if (values.plisio_fx_markup_percent !== undefined) values.plisio_fx_markup_percent = String(Math.max(-20, Math.min(50, Number(values.plisio_fx_markup_percent || 0))));
  ...
  if (sharedMarzbanEnabled(prospectiveMarzbanSettings, env)) {
    try {
      ...
      await applySharedMarzbanTemplateUser(env, prospectiveMarzbanSettings, values, { required: true, forceToken: false });
...
async function deployUploadedGithubVersion(env, chatId, version) {
  ...
  try {
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      result = await automaticUpdateTick(env, { source: "telegram_zip_upload", forceCheck: true, intervalSeconds: 5 });
      if (result?.failed) lastError = new Error(result.error || "انتشار خودکار ناموفق بود");
      if (result?.complete || !result?.update_required) break;
    }
    const stageLabel = ({edge:"Edge",processor:"Processor",core:"Core",none:"کامل",deferred:"در انتظار اتصال"})[String(result?.rollout_stage||"")] || "صف انتشار";
    await telegramApi(env, "sendMessage", {
      chat_id: chatId,
      text: (result?.complete ? "✅" : "🔄") + " <b>انتشار خودکار نسخه " + botEscape(version) + "</b>\n\n" +
        "آخرین مرحله: <b>" + botEscape(stageLabel) + "</b>\n" +
        (result?.complete ? "Core، Edge و Processor همگام شدند." : "انتشار ذخیره شد و در Cron یا بازشدن پنل از همین مرحله ادامه می‌یابد.") +
        (lastError ? "\n\n<code>" + botEscape(cleanText(lastError.message,500)) + "</code>" : ""),
      parse_mode: "HTML"
    });
  } catch (error) {
    try {
      await setSettings(env,{auto_update_last_check_epoch:"0",auto_update_last_status:"failed",auto_update_last_error:cleanText(error?.message||error,900)});
      await telegramApi(env, "sendMessage", {
        chat_id: chatId,
        text: "⚠️ فایل‌ها در GitHub ثبت شدند، اما انتشار خطا داد:\n<code>" + botEscape(cleanText(error?.message || error, 700)) + "</code>",
        parse_mode: "HTML"
      });
    } catch (_) {}
  }
...
async function checkUpdate(env) {
  ...
  const knownCoreSha = String(settings.auto_update_last_worker_sha || "");
  const coreSha = cleanText(manifest.hashes?.core || "", 128);
  const edgeSha = cleanText(manifest.hashes?.edge || "", 128);
  const processorSha = cleanText(manifest.hashes?.processor || "", 128);
  const releaseChanged = Boolean(manifest.release_id && manifest.release_id !== knownReleaseId);
  const coreBuildChanged = Boolean(coreSha && coreSha !== knownCoreSha);
  const buildAvailable = Boolean(versionOrder === 0 && (releaseChanged || coreBuildChanged));
  const canSyncRemote = versionOrder <= 0;
  const cloudflareConfigured = Boolean(settings.cf_account_id && settings.cf_api_token && settings.cf_worker_name);
  const edgeTargetConfigured = Boolean(settings.edge_worker_script_name || hasEdgeServiceBinding(env) || cloudflareConfigured);
  const processorTargetConfigured = Boolean(settings.processor_worker_script_name || hasProcessorServiceBinding(env) || cloudflareConfigured);
  const edgeSyncRequired = Boolean(canSyncRemote && edgeTargetConfigured && (
    String(settings.edge_worker_last_version || "") !== latest ||
    Boolean(edgeSha && String(settings.edge_worker_last_sha || "") !== edgeSha)
  ));
  const processorSyncRequired = Boolean(canSyncRemote && processorTargetConfigured && (
    String(settings.processor_worker_last_version || "") !== latest ||
    Boolean(processorSha && String(settings.processor_worker_last_sha || "") !== processorSha)
  ));
  ...
  return {
    ...
    edge_target_configured: edgeTargetConfigured,
    processor_target_configured: processorTargetConfigured
  };
}
...
async function deployClusterFromGithub(env, force = false, prechecked = null) {
  ...
  let edge_update = { skipped: true, reason: "already_synced" };
  let processor_update = { skipped: true, reason: "already_synced" };
  let core = { ...check, deployed: false };
  if (!check.update_required && !force) {
    return { ...check, deployed: false, complete: true, rollout_stage: "none", edge_update, processor_update };
  }
  ...
  if (trackingReady) {
    ...
    ) VALUES(?,?,?,'resumable','running','pending','pending','pending',?,?,?,?,?,?)`, [
      rolloutId,check.latest||APP_VERSION,check.release_id||"",APP_VERSION,
      settings.edge_worker_last_version||"",settings.processor_worker_last_version||"",JSON.stringify({force,reason:check.reason}),ts,ts
    ]);
  }

  // One Worker per invocation keeps ZIP waitUntil and normal panel requests below
  ...
  if (check.available || force) {
    try {
      ...
      if (trackingReady) await safeRolloutStatement(env,"UPDATE release_rollouts SET status='failed',core_status='failed',details_json=?,finished_at=?,updated_at=? WHERE id=?",[
        JSON.stringify({edge_update,processor_update,error:cleanText(error?.message||error,1000)}),nowIso(),nowIso(),rolloutId
      ]);
  ...
  // A missing side-worker target must not permanently block Core recovery.
  const checkAfter = await checkUpdate(env).catch(()=>check);
  return { ...checkAfter, deployed:false, complete:!checkAfter.update_required, partial:checkAfter.update_required, rollout_id:rolloutId, rollout_stage:"deferred", tracking_ready:trackingReady, core_deployed:false, edge_update, processor_update };
}
...
async function automaticUpdateTick(env, options = {}) {
  ...
  try {
    ...
    await setSettings(env, {
      auto_update: "true",
      auto_update_last_status: result.complete ? (result.deployed ? "deployed" : "up_to_date") : "deploying",
      auto_update_last_version: result.core_deployed ? (result.latest || APP_VERSION) : (settings.auto_update_last_version || APP_VERSION),
...
async function tryImmediateReportDelivery(env, reportId, botId, topicKey, title, messageHtml) {
  ...
  let resellerStatus = botId ? "pending" : "skipped";
  const errors = [];
  let bot = null;
  if (botId) {
    try {
  ...
  const centralDefinition = reportTopicDefinition("central", topicKey);
  const resellerDefinition = reportTopicDefinition("reseller", topicKey);
  const botHeader = bot
  ...
  try {
    const destination = await env.PASARGUARD_DB.prepare(`SELECT rf.chat_id,rft.thread_id
      FROM report_forums rf LEFT JOIN report_forum_topics rft ON rft.scope_key=rf.scope_key AND rft.topic_key=?
      WHERE rf.scope_key='central' AND rf.status='active'`).bind(centralDefinition.key).first();
    if (!destination) centralStatus = "skipped";
    else if (!destination.thread_id) errors.push("مرکزی: Topic ثبت نشده است");
    else {
      await telegramApi(env, "sendMessage", {
        chat_id: String(destination.chat_id), message_thread_id: Number(destination.thread_id), parse_mode: "HTML",
        disable_web_page_preview: true,
        text: cleanText("<b>" + botEscape(title) + "</b>\n━━━━━━━━━━━━━━\n" + botHeader + messageHtml, 3900)
      });
      centralStatus = "delivered";
      await env.PASARGUARD_DB.prepare("UPDATE report_forums SET last_delivery_at=?,last_error=NULL,updated_at=? WHERE scope_key='central'")
        .bind(nowIso(), nowIso()).run();
    }
  } catch (error) {
    errors.push("مرکزی: " + String(error?.message || error));
  }
  if (botId) {
    try {
      const scopeKey = reportScopeKey(botId);
      const destination = await env.PASARGUARD_DB.prepare(`SELECT rf.chat_id,rft.thread_id
        FROM report_forums rf LEFT JOIN report_forum_topics rft ON rft.scope_key=rf.scope_key AND rft.topic_key=?
        WHERE rf.scope_key=? AND rf.status='active'`).bind(resellerDefinition.key, scopeKey).first();
      if (!destination) resellerStatus = "skipped";
      else if (!destination.thread_id) errors.push("نماینده: Topic ثبت نشده است");
      else if (!bot) errors.push("نماینده: ربات پیدا نشد");
      else {
        ...
        resellerStatus = "delivered";
        await env.PASARGUARD_DB.prepare("UPDATE report_forums SET last_delivery_at=?,last_error=NULL,updated_at=? WHERE scope_key=?")
          .bind(nowIso(), nowIso(), scopeKey).run();
      }
    } catch (error) {
      errors.push("نماینده: " + String(error?.message || error));
    }
  }
  await env.PASARGUARD_DB.prepare(`UPDATE report_outbox SET central_status=?,reseller_status=?,last_error=?,updated_at=? WHERE id=?`)
    .bind(centralStatus, resellerStatus, errors.length ? cleanText(errors.join(" | "), 600) : null, nowIso(), reportId).run();
...
async function handleCentralReportGroupCommand(env, message, settings) {
  ...
  if (isTest) {
    const testId = "report-test:central:" + Date.now();
    const queued = await queueReportEvent(env, null, "misc", "گزارش آزمایشی مرکزی",
      "✅ اتصال گروه، Topic و ارسال گزارش با موفقیت بررسی شد.\nزمان: <code>" + botEscape(nowIso()) + "</code>", testId);
    await telegramApi(env, "sendMessage", {
      chat_id: message.chat.id, parse_mode: "HTML",
      text: queued?.delivery?.centralStatus === "delivered"
        ? "✅ گزارش آزمایشی در Topic «سایر گزارشات» ارسال شد."
        : "⚠️ گزارش آزمایشی وارد صف شد اما ارسال فوری انجام نشد. /reports_status را بررسی کنید."
    });
    return true;
  }
  if (isStatus) {
  ...
  try { await triggerProcessorBusinessJobs(env, "report_setup"); } catch (_) {}
  return true;
}
...
function safeSharedMarzbanTemplateProxies(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const result = {};
  for (const [rawProtocol, rawSettings] of Object.entries(source)) {
    const protocol = String(rawProtocol || "").toLowerCase();
    if (!["vless", "vmess", "trojan", "shadowsocks"].includes(protocol)) continue;
    const settings = rawSettings && typeof rawSettings === "object" && !Array.isArray(rawSettings) ? rawSettings : {};
    // Never copy credentials from the template account. Marzban generates a new UUID/password.
    if (protocol === "vless" && settings.flow) result[protocol] = { flow: cleanText(settings.flow, 100) };
    else if (protocol === "shadowsocks" && settings.method) result[protocol] = { method: cleanText(settings.method, 100) };
    else result[protocol] = {};
  }
  return result;
}

function safeSharedMarzbanTemplateInbounds(value, proxies) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const result = {};
  for (const protocol of Object.keys(proxies || {})) {
    const tags = Array.isArray(source[protocol]) ? source[protocol].map(x => cleanText(x, 240)).filter(Boolean) : [];
    if (tags.length) result[protocol] = [...new Set(tags)];
  }
  return result;
}

async function resolveSharedMarzbanTemplateUser(env, settings, value, options = {}) {
  const templateUsername = normalizeSharedMarzbanTemplateUsername(value);
  if (!templateUsername) return { username: "", proxies: {}, inbounds: {} };
  const cfg = sharedMarzbanConfig(settings, env);
  const token = await getSharedMarzbanToken(env, settings, Boolean(options.forceToken));
  const response = await fetch(cfg.panelUrl + "/api/user/" + encodeURIComponent(templateUsername), {
    headers: { authorization: "Bearer " + token, accept: "application/json" }
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const detail = data?.detail || data?.message || data?.error || ("HTTP " + response.status);
    throw new Error("اشتراک نمونه «" + templateUsername + "» در مرزبان خوانده نشد: " + (typeof detail === "string" ? detail : JSON.stringify(detail)));
  }
  const proxies = safeSharedMarzbanTemplateProxies(data?.proxies);
  const inbounds = safeSharedMarzbanTemplateInbounds(data?.inbounds, proxies);
  if (!Object.keys(proxies).length || !Object.keys(inbounds).length) {
    throw new Error("اشتراک نمونه «" + templateUsername + "» هیچ Inbound فعال و قابل استفاده‌ای ندارد");
  }
  for (const protocol of Object.keys(proxies)) {
    if (!Array.isArray(inbounds[protocol]) || !inbounds[protocol].length) delete proxies[protocol];
  }
  if (!Object.keys(proxies).length) throw new Error("پروتکل و Inbound قابل استفاده‌ای از اشتراک نمونه پیدا نشد");
  return { username: cleanText(data?.username || templateUsername, 200), proxies, inbounds };
}

async function applySharedMarzbanTemplateUser(env, settings, values = {}, options = {}) {
  const effective = { ...settings, ...values };
  const username = normalizeSharedMarzbanTemplateUsername(
    values.marzban_template_username !== undefined ? values.marzban_template_username : effective.marzban_template_username
  );
  if (!username) {
    if (options.required) throw new Error("نام کاربری یک اشتراک نمونه موجود در مرزبان را وارد کنید");
    return { values, profile: null };
  }
  const profile = await resolveSharedMarzbanTemplateUser(env, effective, username, options);
  values.marzban_template_username = profile.username;
  values.marzban_proxies_json = JSON.stringify(profile.proxies);
  values.marzban_inbounds_json = JSON.stringify(profile.inbounds);
...
async function botSalesBotView(env, account) {
  ...
  rows.push([{ text: "🔑 تغییر توکن ربات نماینده", callback_data: "bot:sales:token" }]);
...
async function botAdminPlisioView(env, account) {
  ...
  const fxKeySet = !!String(s.plisio_fx_api_key || "").trim();
  const enabled = s.plisio_enabled === "true";
  const mode = plisioRateMode(s);
  const currentRate = mode === "auto" ? Number(s.plisio_fx_last_rate_toman || 0) : Number(s.plisio_toman_per_source_unit || 0);
  const ready = keySet && (currentRate > 0 || (mode === "auto" && fxKeySet));
  const lines = [
    "🪙 <b>درگاه رمزارزی Plisio</b>",
    "━━━━━━━━━━━━━━",
    "وضعیت: <b>" + (enabled && ready ? "🟢 فعال" : enabled ? "🟠 تنظیمات ناقص" : "🔴 غیرفعال") + "</b>",
    "Secret Key: <b>" + (keySet ? "ثبت و رمزگذاری شده" : "ثبت نشده") + "</b>",
    "ارز مبنا: <b>USD</b>",
    "روش نرخ: <b>" + (mode === "auto" ? "خودکار از ناواسان" : "دستی") + "</b>",
    "نرخ فعلی: <b>" + botMoney(currentRate) + " تومان</b>"
  ];
  if (mode === "auto") {
    lines.push("API نرخ ارز: <b>" + (fxKeySet ? "ثبت و رمزگذاری شده" : "ثبت نشده") + "</b>");
    lines.push("بروزرسانی: هر <b>" + botMoney(s.plisio_fx_refresh_minutes || 15) + " دقیقه</b>");
  }
  if (s.plisio_fx_last_fetched_at) lines.push("آخرین دریافت نرخ: <b>" + botDate(s.plisio_fx_last_fetched_at) + "</b>");
  lines.push("نرخ دستی پشتیبان: <b>" + botMoney(s.plisio_toman_per_source_unit || 0) + " تومان</b>");
  lines.push("رمزارزهای مجاز: <code>" + botEscape(s.plisio_allowed_currencies || "انتخاب از تنظیمات حساب Plisio") + "</code>");
  lines.push("انقضای فاکتور: <b>" + botMoney(s.plisio_expire_minutes || 60) + " دقیقه</b>");
  if (s.plisio_fx_last_error) lines.push("", "⚠️ آخرین خطای نرخ ارز:", "<code>" + botEscape(s.plisio_fx_last_error) + "</code>");
  if (s.plisio_last_error) lines.push("", "⚠️ آخرین خطای Plisio:", "<code>" + botEscape(s.plisio_last_error) + "</code>");
  lines.push("", "در حالت خودکار، نرخ دلار هنگام ساخت فاکتور بررسی می‌شود؛ در قطعی منبع، آخرین نرخ سالم و سپس نرخ دستی پشتیبان استفاده می‌شود.");
  return {
    text: lines.join("\n"),
    reply_markup:{inline_keyboard:[
      [{text:keySet?"🔑 تغییر Secret Key":"🔑 ثبت Secret Key",callback_data:"bot:admin:plisio:key"}],
      [{text:fxKeySet?"💵 تغییر API نرخ دلار":"💵 ثبت API نرخ دلار",callback_data:"bot:admin:plisio:fxkey"},{text:"🔄 بروزرسانی نرخ",callback_data:"bot:admin:plisio:fxrefresh"}],
      [{text:mode === "auto" ? "✍️ تغییر به نرخ دستی" : "🤖 تغییر به نرخ خودکار",callback_data:"bot:admin:plisio:ratemode"}],
      [{text:"🛟 نرخ دستی پشتیبان",callback_data:"bot:admin:plisio:rate"},{text:"⚙️ بازه بروزرسانی",callback_data:"bot:admin:plisio:fxoptions"}],
      [{text:"🪙 رمزارزهای مجاز",callback_data:"bot:admin:plisio:currencies"},{text:"⏱ زمان انقضا",callback_data:"bot:admin:plisio:expire"}],
      [{text:"🧪 بررسی کامل",callback_data:"bot:admin:plisio:test"},{text:enabled?"⏸ غیرفعال‌کردن":"▶️ فعال‌کردن",callback_data:"bot:admin:plisio:toggle"}],
...
async function botAdminMarzbanView(env, account) {
  ...
  const settings = await getSettings(env);
  const enabled = sharedMarzbanEnabled(settings, env);
  const panelUrl = normalizeSharedMarzbanPanelUrl(settings.marzban_panel_url || "");
  const dashboardUrl = sharedMarzbanDashboardUrl(panelUrl);
  ...
  const templateUsername = normalizeSharedMarzbanTemplateUsername(settings.marzban_template_username || "");
  const ready = Boolean(panelUrl && username && passwordSet && templateUsername.length);
  const rows = [
    [{ text: enabled ? "⏸ غیرفعال‌کردن مرزبان" : "▶️ فعال‌کردن مرزبان", callback_data: "bot:admin:marzban:toggle" }, { text: "🧪 تست اتصال و اشتراک نمونه", callback_data: "bot:admin:marzban:test" }],
    ...
    [{ text: "📡 نام اشتراک نمونه", callback_data: "bot:admin:marzban:inbounds" }],
  ...
  if (dashboardUrl) rows.push([{ text: "🖥 ورود به داشبورد مرزبان", url: dashboardUrl }]);
  rows.push([{ text: "🔄 بروزرسانی وضعیت", callback_data: "bot:admin:marzban" }]);
  rows.push([{ text: "↩️ تنظیمات سامانه", callback_data: "bot:admin:settings" }]);
  const lines = [
    "🌐 <b>مرزبان مشترک مرکزی</b>", "━━━━━━━━━━━━━━",
    "وضعیت: <b>" + (enabled && ready ? "🟢 فعال" : enabled ? "🟠 تنظیمات ناقص" : "🔴 غیرفعال") + "</b>",
    "API Base: <code>" + botEscape(panelUrl || "ثبت نشده") + "</code>",
    "ورود مدیر: <code>" + botEscape(dashboardUrl || "ثبت نشده") + "</code>",
    "ادمین: <code>" + botEscape(username || "ثبت نشده") + "</code>",
    "رمز: <b>" + (passwordSet ? "ثبت شده و مخفی" : "ثبت نشده") + "</b>",
    "اشتراک نمونه: <b>" + (templateUsername.length ? botEscape(templateUsername) : "ثبت نشده") + "</b>",
  ...
  lines.push("", "نامی که وارد می‌کنی باید نام کاربری دقیق یک اشتراک موجود در مرزبان باشد. اطلاعات ورود و لینک داشبورد فقط برای مدیر مرکزی نمایش داده می‌شوند.");
  return { text: lines.join("\n"), reply_markup: { inline_keyboard: rows } };
}
...
async function botCreatePlisioPaymentInvoice(env, user, amountToman, origin) {
  ...
  if (amount < minRecharge) throw new Error("حداقل مبلغ شارژ " + botMoney(minRecharge) + " تومان است");
  const localId = id("ppay");
  const fx = await refreshCentralPlisioFxRate(env);
  const sourceAmount = plisioSourceAmount(amount, fx.rate_toman);
  const urls = centralPlisioUrls(origin);
  const params = {
    source_currency: cleanText(settings.plisio_source_currency || "USD", 20).toUpperCase() || "USD",
    source_amount: sourceAmount.toFixed(2),
  ...
  await env.PASARGUARD_DB.prepare(`INSERT INTO payment_invoices(id,user_id,provider,provider_invoice_id,amount_rial,amount_toman,final_amount_rial,status,payment_link,card_number,provider_payload,created_at,updated_at) VALUES(?,?,'plisio',?,?,?,?,'PENDING',?,NULL,?,?,?)`)
    .bind(localId,user.id,providerInvoiceId,amountRial,amount,amountRial,paymentLink,JSON.stringify({request:params,response:provider,fx:{rate_toman:fx.rate_toman,source:fx.source,stale:!!fx.stale,fetched_at:fx.fetched_at||""}}),ts,ts).run();
  await recordCentralPlisioState(env, true);
  await audit(env,user.id,"bot_plisio_invoice_created",{paymentId:localId,providerInvoiceId,amountToman:amount,sourceAmount,fxRateToman:fx.rate_toman,fxSource:fx.source});
...
async function botHandleCallback(env, callback, origin, preloadedSettings = null) {
  ...
  if (data === "bot:admin:marzban:toggle") {
    ...
    if (enabling) {
      try {
        ...
        sharedMarzbanConfig(probeSettings, env);
        await getSharedMarzbanToken(env, probeSettings, true);
        const applied = await applySharedMarzbanTemplateUser(env, probeSettings, {}, { required:true });
        if (Object.keys(applied.values).length) await setSettings(env, applied.values);
      } catch (error) {
  ...
  if (data === "bot:admin:marzban:test") {
    ...
    try {
      ...
      const applied=await applySharedMarzbanTemplateUser(env,probeSettings,{}, {required:true});
      const ts=nowIso();await setSettings(env,{...applied.values,marzban_last_verified_at:ts,marzban_last_error:""});
  ...
  if (["url","username","password","billing","inbounds"].some(x=>data==="bot:admin:marzban:"+x)) {
    if (!account.isAdmin) throw new Error("دسترسی مدیر لازم است");
    const field=data.split(":").pop();
    const prompts={
      url:"🌐 <b>آدرس HTTPS پنل مرزبان را ارسال کنید</b>\nهر دو حالت پذیرفته می‌شود:\n<code>https://panel.example.com</code>\n<code>https://panel.example.com/dashboard</code>",
      ...
      inbounds:"📡 <b>نام کاربری یک اشتراک موجود در مرزبان را ارسال کنید</b>\nربات Proxies و Inbounds همین اشتراک را به‌عنوان الگو می‌خواند.\nUUID، رمز و شناسه‌های امنیتی اشتراک نمونه کپی نمی‌شوند.\nنمونه: <code>sample_user</code>"
    };
  ...
  if (data === "bot:admin:errors:resolve") {
    if(!account.isAdmin) throw new Error("دسترسی مدیر لازم است");
    await resolveSystemErrorCenter(env);
...
لطفاً چند لحظه صبر کنید.`,reply_markup:{inline_keyboard:[]}}); } catch (_) {}
  ...
  if (data === "bot:admin:plisio:fxrefresh") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");const r=await refreshCentralPlisioFxRate(env,{force:true});await botPrompt(env,chatId,"✅ نرخ دلار بروزرسانی شد.\nنرخ: <b>"+botMoney(r.rate_toman)+" تومان</b>\nمنبع: <b>"+botEscape(r.source)+"</b>",[[{text:"↩️ Plisio",callback_data:"bot:admin:plisio"}]]);return; }
  if (data === "bot:admin:plisio:ratemode") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");const st=await getSettings(env),next=plisioRateMode(st)==="auto"?"manual":"auto";if(next==="manual"&&Number(st.plisio_toman_per_source_unit||0)<=0)throw new Error("ابتدا نرخ دستی پشتیبان را ثبت کنید");await setSettings(env,{plisio_rate_mode:next});const fresh=await ensureTelegramBotUser(env,callback.from);fresh.public_origin=origin;await botSendOrEdit(env,{account:fresh,chatId,messageId},"admin_plisio");return; }
  if (data === "bot:admin:plisio:fxoptions") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");await botSetSession(env,account.user.telegram_id,"setting_plisio_fx_options",{});await botPrompt(env,chatId,"⚙️ بازه بروزرسانی و درصد حاشیه نرخ را با | ارسال کنید.\nمثال: <code>15 | 0</code>\nعدد اول دقیقه (۵ تا ۱۴۴۰) و عدد دوم درصد (-۲۰ تا ۵۰) است.");return; }
  if (data === "bot:admin:plisio:rate") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");await botSetSession(env,account.user.telegram_id,"setting_plisio_rate",{});await botPrompt(env,chatId,"🛟 نرخ دستی پشتیبان دلار را به تومان ارسال کنید.\nمثال: <code>100000</code>");return; }
  ...
  if (data === "bot:admin:plisio:test") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");try{const r=await testCentralPlisioConnection(env);await botPrompt(env,chatId,"✅ تنظیمات Plisio و نرخ ارز معتبر است.\nارز مبنا: <b>"+botEscape(r.source_currency)+"</b>\nنرخ دلار: <b>"+botMoney(r.rate_toman)+" تومان</b>\nمنبع نرخ: <b>"+botEscape(r.rate_source)+"</b>\nتأیید پرداخت‌ها از Callback امضاشده انجام می‌شود.",[[{text:"↩️ Plisio",callback_data:"bot:admin:plisio"}]]);}catch(e){await recordCentralPlisioState(env,false,e.message);throw e}return; }
  if (data === "bot:admin:plisio:toggle") { if(!account.isAdmin)throw new Error("دسترسی مدیر لازم است");const st=await getSettings(env),next=st.plisio_enabled==="true"?"false":"true";if(next==="true"&&(!st.plisio_api_key||(!plisioAutoRateConfigured(st)&&Number(st.plisio_toman_per_source_unit||0)<=0)))throw new Error("ابتدا Secret Key و تنظیم نرخ خودکار یا نرخ دستی پشتیبان را ثبت کنید");await setSettings(env,{plisio_enabled:next});const fresh=await ensureTelegramBotUser(env,callback.from);fresh.public_origin=origin;await botSendOrEdit(env,{account:fresh,chatId,messageId},"admin_plisio");return; }
  ...
  if (data === "bot:sales:token") {
    const bot = await getResellerBotForUser(env, account.user.id);
    if (!bot) throw new Error("ربات نماینده پیدا نشد");
    await botSetSession(env, account.user.telegram_id, "sales_bot_change_token", { botId: bot.id });
    await botPrompt(env, chatId,
      "🔑 <b>تغییر توکن ربات نماینده</b>\n━━━━━━━━━━━━━━\nتوکن جدید BotFather را ارسال کنید.\n\n✅ کاربران، سفارش‌ها، پلن‌ها، موجودی‌ها و تنظیمات حفظ می‌شوند.\n✅ توکن با Telegram بررسی و Webhook خودکار منتقل می‌شود.\n⚠️ پیام حاوی توکن پس از بررسی حذف خواهد شد.",
...
async function botHandleSessionMessage(env, account, message, session, origin, ctx = null) {
  ...
  if (session.state === "sales_bot_change_token") {
    ...
    if (!bot) throw new Error("ربات نماینده پیدا نشد");
    try { await telegramApi(env, "deleteMessage", { chat_id: chatId, message_id: message.message_id }); } catch (_) {}
    const changed = await replaceResellerBotToken(env, bot, text, { origin });
    await botClearSession(env, account.user.telegram_id);
    try {
      await audit(env, account.user.id, "reseller_bot_token_changed", {
        botId: bot.id, oldTelegramId: changed.old_telegram_id, newTelegramId: changed.bot.bot_telegram_id, username: changed.username
      });
    } catch (_) {}
    try {
      await queueReportEvent(env, bot.id, "bots", "توکن ربات نماینده تغییر کرد",
        "ربات جدید: <b>@" + botEscape(changed.username) + "</b>\nمالک: <code>" + botEscape(account.user.telegram_id) + "</code>",
        "bot-token-changed:" + bot.id + ":" + changed.bot.bot_telegram_id
      );
    } catch (_) {}
    await botPrompt(env, chatId,
      "✅ <b>توکن ربات نماینده با موفقیت تغییر کرد</b>\n━━━━━━━━━━━━━━\nربات فعال: <b>@" + botEscape(changed.username) + "</b>\nWebhook و مینی‌اپ دوباره تنظیم شدند.\n\nتمام کاربران، سفارش‌ها، پلن‌ها، کیف پول‌ها و تنظیمات قبلی حفظ شدند." +
      (changed.changed_bot ? "\n\n⚠️ ربات جدید را یک‌بار باز کنید و Start بزنید." : ""),
      [[{ text: "🚀 بازکردن ربات", url: "https://t.me/" + changed.username }], [{ text: "↩️ مدیریت ربات", callback_data: "bot:sales" }]]
    );
    return true;
  }
  ...
  if (String(session.state || "").startsWith("setting_marzban_")) {
    ...
    if (field==="url") {
      const url=normalizeSharedMarzbanPanelUrl(text);
      ...
      const templateUsername=normalizeSharedMarzbanTemplateUsername(text);
      if(!templateUsername)throw new Error("نام کاربری اشتراک نمونه را وارد کنید");
      const prospective={...settings,marzban_template_username:templateUsername,marzban_shared_enabled:"true",service_backend:"marzban_shared"};
      const profile=await resolveSharedMarzbanTemplateUser(env,prospective,templateUsername,{forceToken:true});
      values.marzban_template_username=profile.username;
      values.marzban_proxies_json=JSON.stringify(profile.proxies);
      values.marzban_inbounds_json=JSON.stringify(profile.inbounds);
  ...
  if (["setting_plisio_key","setting_plisio_fx_key","setting_plisio_rate","setting_plisio_fx_options","setting_plisio_currencies","setting_plisio_expire"].includes(session.state)) {
    ...
    if(session.state==="setting_plisio_fx_key"){if(text.length<8)throw new Error("API Key نرخ ارز معتبر نیست");values.plisio_fx_api_key="enc:"+await encryptSecret(text,env);values.plisio_rate_mode="auto";values.plisio_fx_last_error="";try{await telegramApi(env,"deleteMessage",{chat_id:chatId,message_id:message.message_id})}catch(_){}}
    if(session.state==="setting_plisio_rate"){const rate=botParseInteger(text.split("|").pop());if(!Number.isFinite(rate)||rate<=0)throw new Error("نرخ تومان معتبر نیست");values.plisio_source_currency="USD";values.plisio_toman_per_source_unit=String(rate);}
    if(session.state==="setting_plisio_fx_options"){const p=text.split("|").map(x=>x.trim()),minutes=botParseInteger(p[0]),markup=Number(normalizeDigitsForNumber(p[1]||"0"));if(!Number.isFinite(minutes)||minutes<5||minutes>1440)throw new Error("بازه بروزرسانی باید بین ۵ تا ۱۴۴۰ دقیقه باشد");if(!Number.isFinite(markup)||markup<-20||markup>50)throw new Error("درصد حاشیه باید بین منفی ۲۰ تا مثبت ۵۰ باشد");values.plisio_fx_refresh_minutes=String(minutes);values.plisio_fx_markup_percent=String(markup);}
...
async function configureTelegramBotWithToken(env, origin, token, suppliedSettings = null) {
  ...
  await telegramApiWithToken(token, "setChatMenuButton", { menu_button: { type: "commands" } });
...
async function telegramWebhook(request, env, ctx = null) {
  ...
  try {
    ...
    const replyRoute = !text.startsWith("/") ? await telegramFindReplyRoute(env, "central", message.chat.id, text) : null;
    if (replyRoute) {
      const rawSend = (method, body) => telegramApi(env, method, body);
      if (await telegramSendReplyRouteSpecial(rawSend, message.chat.id, replyRoute)) return json({ ok: true });
      if (replyRoute.action?.kind === "callback" && replyRoute.action.data) {
        await botHandleCallback(env, {
          id: "reply-keyboard-" + Date.now(),
          from: message.from,
          data: String(replyRoute.action.data),
          message: { ...message, message_id: replyRoute.source_message_id || message.message_id }
        }, origin, settings);
        return json({ ok: true });
      }
    }
...
async function centralAdminAction(request, env) {
  ...
  try {
    if (action === "save_shared_marzban") {
      ...
      const panelUrl=normalizeSharedMarzbanPanelUrl(body.panel_url ?? current.marzban_panel_url ?? "");
      ...
      const templateUsername=normalizeSharedMarzbanTemplateUsername(body.template_username ?? body.marzban_template_username ?? current.marzban_template_username ?? "");
      const values={
        ...
        marzban_template_username:templateUsername,
      ...
      if(enabled){
        ...
        try{sharedMarzbanConfig(prospective,env);await getSharedMarzbanToken(env,prospective,true);await applySharedMarzbanTemplateUser(env,prospective,values,{required:true});values.marzban_last_verified_at=nowIso();values.marzban_last_error="";}
    ...
    if (action === "test_shared_marzban") {
      const settings=await getSettings(env);
      const probeSettings={...settings,marzban_shared_enabled:"true",service_backend:"marzban_shared"};
      try{const token=await getSharedMarzbanToken(env,probeSettings,true);const applied=await applySharedMarzbanTemplateUser(env,probeSettings,{}, {required:true});const ts=nowIso();await setSettings(env,{...applied.values,marzban_last_verified_at:ts,marzban_last_error:""});await audit(env,auth.user.id,"shared_marzban_tested_from_central_panel",{ok:Boolean(token),panel_url:settings.marzban_panel_url||""});return json({success:true,message:"اتصال مرزبان موفق بود",result:{verified_at:ts}});}
    ...
    if (action === "update_deploy") {
      const result = await deployClusterFromGithub(env, body.force === true); await audit(env, auth.user.id, "central_dashboard_update_deploy", result); return json({ success: true, message: result.complete ? (result.deployed ? "انتشار بروزرسانی کامل شد" : "تغییری برای نصب وجود نداشت") : "مرحله " + String(result.rollout_stage||"بعدی") + " انجام شد؛ ادامه انتشار خودکار است", result });
    }
